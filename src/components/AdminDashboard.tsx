...
const AdminDashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Set up real-time subscriptions for bank-specific tables
    const maybankChannel = supabase
      ...
      .subscribe((status) => {
        console.log('Maybank subscription status:', status);
      });
    ...
    const bpiChannel = supabase
      ...
      .on(
        ...
        () => {
          console.log('Real-time update received for BPI applications');
          fetchBankApplications();
        }
      )
      .subscribe();
    ...
    const eastwestChannel = supabase
      ...
      .on(
        ...
        () => {
          console.log('Real-time update received for EastWest applications');
          fetchBankApplications();
        }
      )
      .subscribe();
    ...
    const rcbcChannel = supabase
      ...
      .on(
        ...
        () => {
          console.log('Real-time update received for RCBC applications');
          fetchBankApplications();
        }
      )
      .subscribe();
    ...
    const pnbChannel = supabase
      ...
      .on(
        ...
        () => {
          console.log('Real-time update received for PNB applications');
          fetchBankApplications();
        }
      )
      .subscribe();
    ...
    const aubChannel = supabase
      ...
      .on(
        ...
        () => {
          console.log('Real-time update received for AUB applications');
          fetchBankApplications();
        }
      )
      .subscribe();
    ...
    const metrobankChannel = supabase
      ...
      .on(
        ...
        () => {
          console.log('Real-time update received for Metrobank applications');
          fetchBankApplications();
        }
      )
      .subscribe();
    ...
    const robinsonsChannel = supabase
      ...
      .on(
        ...
        () => {
          console.log('Real-time update received for Robinson Bank applications');
          fetchBankApplications();
        }
      )
      .subscribe();
                    ...
                    <tr key={app.id || app.applicationNo || app.application_no} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex space-x-2">
                          ...
                          {/* Display status badge for bank-specific applications */}
                          <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                            app.status === 'pending' ? 'bg-yellow-100 text-yellow-800' :
                            app.status === 'approved' ? 'bg-green-100 text-green-800' :
                            app.status === 'rejected' ? 'bg-red-100 text-red-800' :
                            app.status === 'cancelled' ? 'bg-gray-100 text-gray-800' :
                            app.status === 'incomplete' ? 'bg-orange-100 text-orange-800' :
                            app.status === 'in_process' ? 'bg-blue-100 text-blue-800' :
                            app.status === 'existing' ? 'bg-purple-100 text-purple-800' :
                            'bg-gray-100 text-gray-600'
                          }`}>
                            {app.status || 'pending'}
                          </span>
                          
                          {/* Show delete button for bank status entries */}
                          {(() => {
                    ...
                    <tr key={app.id || app.applicationNo || app.application_no} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex space-x-2">
                          ...
                          {/* Display status badge for bank-specific applications */}
                          <span className={`px-2 py-1 rounded-full text-xs font-semibold ${app.status === 'pending' ? 'bg-yellow-100 text-yellow-800' : 
                                           app.status === 'approved' || app.status === 'accepted' ? 'bg-green-100 text-green-800' : 
                ...
                .map((app) => (
                  <div key={app.id || app.applicationNo || app.application_no} className="border-b border-gray-200 p-4 hover:bg-gray-50">
                    <div className="flex justify-between items-start mb-3">
                      ...
                      <span className={`px-2 py-1 rounded-full text-xs font-semibold ml-2 ${app.status === 'pending' ? 'bg-yellow-100 text-yellow-800' : 
                                       app.status === 'approved' || app.status === 'accepted' ? 'bg-green-100 text-green-800' : 
                                       app.status === 'rejected' ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-600'}`}>
                        {app.status || 'pending'}
                      </span>
